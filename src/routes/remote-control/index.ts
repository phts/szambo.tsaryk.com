import {Route} from '..'
import {NewScheduledAction, RemoteControl, RemoteControlAction} from '../../models'
  ...
  async (req, res) => {
    ...
    if (when === 'now') {
      ...
      await services.logs.insertOne({
        message: `Remote action "${action}" requested (id=${generateId(item)})`,
        severity: 'info',
      })
      ...
      const item = {when: datetime, action}
      await exec<NewScheduledAction>('scheduled-actions', async (collection) => {
        await collection.insertOne(item)
      })

      await services.logs.insertOne({
        message: `Remote action "${action}" scheduled on ${datetime.toLocaleString()}`,
        severity: 'info',
      })
    }
